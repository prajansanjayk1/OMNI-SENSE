...
const Screen0_DashboardOverview = ({ 
  ...
  const handleRunScenario = async () => {
    ...
    try {
      const res = await fetch('http://localhost:8000/api/scenario-simulator', {
        method: 'POST',
        headers: {
  ...
  const handleFetchDigitalTwin = async () => {
    ...
    try {
      const res = await fetch('http://localhost:8000/api/borrower/BALAJI-001/digital-twin', {
  ...
  const handleFetchGraph = async () => {
    ...
    try {
      const res = await fetch('http://localhost:8000/api/knowledge-graph?corporate_id=BALAJI-001', {
        headers: { 'Authorization': `Bearer ${token}` }
      });
      if (res.ok) {
  ...
  const fetchDashboardData = React.useCallback(async () => {
    ...
    try {
      ...
      // 1. Budget
      try {
        const res = await fetch('http://localhost:8000/api/customer/budget', { headers });
        if (res.ok) {
          const data = await res.json();
      ...
      // 2. Savings Goals
      try {
        const res = await fetch('http://localhost:8000/api/customer/goals', { headers });
      ...
      if (role === 'Customer') {
        try {
          const res = await fetch('http://localhost:8000/api/customer/insights', { headers });
          if (res.ok) {
            const data = await res.json();
        ...
        try {
          const res = await fetch('http://localhost:8000/api/loans/applications', { headers });
          if (res.ok) setLoans(await res.json());
        } catch (e) {}
        
        try {
          const res = await fetch('http://localhost:8000/api/support/tickets', { headers });
      ...
      if (['Customer', 'Financial Analyst', 'Super Admin'].includes(role)) {
        try {
          const res = await fetch('http://localhost:8000/api/financial/wealth', { headers });
          if (res.ok) {
            const data = await res.json();
      ...
      if (role === 'Super Admin') {
        ...
        try {
          const res = await fetch('http://localhost:8000/api/auth/audit_logs', { headers });
          if (res.ok) setAuditLogs(await res.json());
        } catch (e) {}

        try {
          const res = await fetch('http://localhost:8000/api/admin/system-health', { headers });
          if (res.ok) setSystemHealth(await res.json());
        } catch (e) {}

        try {
          const res = await fetch('http://localhost:8000/api/admin/ai-monitoring', { headers });
  ...
  const handleSaveBudget = async () => {
    ...
    try {
      const res = await fetch('http://localhost:8000/api/customer/budget', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          'Authorization': `Bearer ${token}`
  ...
  const handleCreateGoal = async (e) => {
    ...
    try {
      const res = await fetch('http://localhost:8000/api/customer/goals', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          'Authorization': `Bearer ${token}`
  ...
  const handleDepositGoal = async (e) => {
    ...
    try {
      const res = await fetch(`http://localhost:8000/api/customer/goals/${selectedGoalId}/deposit`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          'Authorization': `Bearer ${token}`
  ...
  const handleBackupDatabase = async () => {
    try {
      const res = await fetch('http://localhost:8000/api/admin/backup', {
        method: 'POST',
        headers: {
          'Authorization': `Bearer ${token}`
        }
  ...
  const handleLoanStatus = async (appId, status) => {
    try {
      const res = await fetch(`http://localhost:8000/api/loans/applications/${appId}/status`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          'Authorization': `Bearer ${token}`
